...
const projects = [
  {
    ...
    duration: 'Showreel',
    video: Portfolio,
    gridClass: 'col-span-full h-[500px] md:h-[600px]',
    glowColor: 'group-hover:border-green-600/30 group-hover:shadow-[0_10px_30px_rgba(252,111,3,0.08)]',
  ...
  },
  {
    ...
    glowColor: 'group-hover:border-green-500/30 group-hover:shadow-[0_10px_30px_rgba(252,111,3,0.08)]',
    accent: 'text-green-600 bg-green-50/80 border-green-200/50'
  },
  {
    id: 3,
    ...
    gridClass: 'col-span-1 md:col-span-1 lg:col-span-6 h-[400px]',
    glowColor: 'group-hover:border-green-700/30 group-hover:shadow-[0_10px_30px_rgba(252,111,3,0.08)]',
  ...
  },
  {
    ...
    gridClass: 'col-span-1 md:col-span-1 lg:col-span-6 h-[400px]',
    glowColor: 'group-hover:border-green-400/30 group-hover:shadow-[0_10px_30px_rgba(252,111,3,0.08)]',
    accent: 'text-green-600 bg-green-50/80 border-green-200/50'
  },
  {
    id: 5,
    ...
    gridClass: 'col-span-1 md:col-span-2 lg:col-span-6 h-[400px]',
    glowColor: 'group-hover:border-emerald-600/30 group-hover:shadow-[0_10px_30px_rgba(252,111,3,0.08)]',
    accent: 'text-emerald-700 bg-emerald-50/80 border-emerald-200/50'
  }
];

function ProjectCard({ project, isMotionCard = false }) {
  ...
  return (
    <motion.div
      ...
      data-cursor="view"
    >
      {/* Video - fully visible for Motion Graphics card, partial visibility for others */}
      <video
        ref={videoRef}
        className={`absolute inset-0 w-full h-full object-cover transition-all duration-700 pointer-events-none ${
          isMotionCard
            ? 'opacity-100'
            : 'opacity-[0.08] group-hover:opacity-[0.85] group-hover:scale-105'
        }`}
        loop
        muted
        autoPlay={isMotionCard}
        playsInline
      >
        <source src={project.video} type="video/mp4" />
      </video>

      {/* Dynamic Overlay Gradient: only for non-motion cards */}
      {!isMotionCard && (
      ...
      {/* Hover Light Leak Overlay Effect (Greenish Tint) - only for non-motion cards */}
      {!isMotionCard && (
      ...
      <div className="absolute top-6 left-6 flex items-center gap-2 pointer-events-none z-10">
        <span className={`font-mono text-[10px] px-2.5 py-0.5 rounded-full uppercase tracking-wider border backdrop-blur-sm transition-colors duration-500 ${
          isMotionCard
      ...
      <div className="absolute top-6 right-6 pointer-events-none z-10">
        <span className={`font-mono text-[10px] px-2 py-0.5 rounded backdrop-blur-sm transition-colors duration-500 ${
          isMotionCard
            ? 'text-white bg-black/40 border border-white/20'
            : 'text-slate-600 bg-white/80 border border-slate-200/60 group-hover:text-slate-200 group-hover:bg-slate-900/60 group-hover:border-slate-800'
        }`}>
          {project.duration}
        </span>
      </div>
      ...
      <div className="absolute bottom-6 left-6 right-6 flex justify-between items-end z-10">
        <div className="pointer-events-none">
          <span className={`font-mono text-[10px] tracking-wider block mb-1 transition-colors duration-500 ${
            isMotionCard
              ? 'text-white/70'
          ...
          <h3 className={`font-clash text-2xl md:text-3xl font-semibold tracking-tight leading-tight transition-all duration-500 ${
            isMotionCard
              ? 'text-white'
              : 'text-slate-800 group-hover:text-white group-hover:translate-x-1'
          }`}>
            {project.title}
          </h3>
        </div>
        ...
        <div className={`w-10 h-10 rounded-full border flex items-center justify-center transition-all duration-500 ${
          isMotionCard
            ...
            : 'border-slate-200 bg-white/80 text-slate-800 opacity-0 group-hover:opacity-100 group-hover:rotate-45 group-hover:border-white/20 group-hover:bg-white/20 group-hover:text-white'
        }`}>
...
export default function Projects() {
  return (
    <section
      id="work"
      className="relative pt-6 pb-20 bg-white flex justify-center items-center overflow-hidden"
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12 w-full z-10">

        {/* Header Section */}
        <div className="flex flex-col items-start text-left mb-12 gap-3">
          ...
          <p className="font-general text-slate-600 max-w-xl text-left text-sm md:text-base leading-relaxed mt-2">
